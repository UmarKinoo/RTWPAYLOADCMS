...
function InfoCard({ title, children, className }: InfoCardProps) {
  return (
    ...
    >
      <p className="font-inter font-semibold text-[#4644b8] text-[14px] sm:text-[16px] md:text-[18px] lg:text-[20px] uppercase tracking-wide">
        {title}
      </p>
      <div className="h-px bg-[#d9d9d9] w-full" />
      <div className="font-inter text-[#16252d] text-[14px] sm:text-[16px] md:text-[18px] lg:text-[20px]">
        {children}
      </div>
    </div>
  )
    ...
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <HomepageSection className="pt-28 sm:pt-32 md:pt-36 lg:pt-40 pb-12 sm:pb-16 md:pb-20">
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 xl:gap-10">
          ...
          <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5 lg:gap-6 content-start">
            ...
            <InfoCard title="Tools & Skills" className="sm:col-span-2">
              <ul className="list-disc list-inside space-y-1 text-[14px] sm:text-[16px] md:text-[18px] lg:text-[20px]">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
                  <p className="font-semibold text-[#2c2c2c] text-[16px] sm:text-[18px] md:text-[20px]">
                    {candidate.jobTitle}
                    {candidate.currentEmployer && (
                      <span className="underline">, {candidate.currentEmployer}</span>
                    )}
                  </p>
                  <p className="text-[12px] sm:text-[14px] text-[#9a9a9a] uppercase">
                    {candidate.experienceYears} years experience
                  </p>
                </div>

                <div className="flex flex-wrap gap-2 text-[14px] sm:text-[16px] text-[#9a9a9a] uppercase">
                ...
                <ul className="list-disc list-inside space-y-1 text-[14px] sm:text-[16px] md:text-[18px] lg:text-[20px] text-[#2c2c2c]">
        ...
        <div className="mt-6 sm:mt-8">
          <InfoCard title="Education" className="w-full">
            <div className="space-y-2">
              <p className="font-semibold text-[#2c2c2c] text-[14px] sm:text-[16px]">
      ...
      {similarCandidates.length > 0 && (
        <HomepageSection className="py-12 sm:py-16 md:py-20 bg-[rgba(175,183,255,0.1)]">
          <h2 className="font-inter font-bold text-[#16252d] text-[28px] sm:text-[36px] md:text-[42px] lg:text-[50px] mb-8 sm:mb-10 md:mb-12">
            Similar Candidates
          </h2>
